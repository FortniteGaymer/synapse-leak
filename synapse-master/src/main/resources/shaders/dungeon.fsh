...
//  p.xz = abs (0.5 - mod ((2./2.) * p.xz, 4.));       // (2) ball &  one object
   p.xz = abs (0.5 - mod ((2./3.) * p.xz, 2.));        // (3) bal &  a few objects
 //   p.xz = abs (0.5 - mod ((2./3.) * p.xz, 1.));    //  (4)  Default (standard)

  s = 1.;
  for (int j = 0; j < 4; j ++) {
    p = abs (p) - vec3 (-0.02, 1.98, -0.02);
    f = 2. / clamp (dot (p, p), 0.4, 1.);
    p = f * p - vec3 (0.75, 1., 0.74);
    s *= f;
    p.xz = Rot2D (p.xz, frctAng);
  }
  d = PrRoundBoxDf (p, vec3 (0.1, 5., 0.1), 0.1) / s;       // (1) Ball Only  (36line deleat)
  DMIN (2);
  return dMin;
}
...
{
  float t, tt, c;
  p = abs (0.5 - fract (18. * p));
  c = 0.;
  t = 0.;
  for (float j = 0.; j < 13.; j ++) {
    p = abs (p + 5.) - abs (p - 5.)  - p ;
    p /= clamp (dot (p, p), 0.5, 0.997);
    p = 3. - 1.5 * p;
    if (mod (j, 2.) == 1.) {
      tt = t;
      t = length (p);
...
//////////////////////////////////////////////// float Tx2Pattern
float Tx2Pattern(vec3 p){
    p=abs(0.5-fract(p+20.));
    float es,l ; es=l=0. ;
    for(int i=0; i<16; i++){
        float pl =l;
        l=length(p);  p=abs(p)/dot(p,p)-0.5 ; es +=exp(-1./abs(l-pl)); }
    return es ; }
//////////////////////////////////////
...
vec3 ShowScene (vec3 ro, vec3 rd)
{
  ...
  if (dstObj < dstFar) {
    if (idObj == 2 || idObj ==1 ) {
      ...
      eMid = 3.35 *0.;
      eVib = eMid +  0.5 *  rnd * sin ((64. + rnd) * tCur*0.3);
      for (int j = 0; j < 30; j ++) {
        rg = ro + dstGlow * rd;
        d = ObjDf (rg);
        dstGlow += d;
        bGlow += max (0., 0.02 - d) * exp (- 0.5 * dstGlow);
        eGlow += max (0., 0.03 - d) * (pow (max (0., 1. - 7. * abs (eVib - rg.x /* rg.y */ )), 8.) +
           pow (max (0., 1. - 20. * min (abs (eMid - rg.x /*rg.y */ - eGap), abs (eMid - rg.x /*rg.y */ + eGap))), 4.));
    ...
    vn = ObjNf (ro);
    amb = vec3 (0.2);
    if (idObjT == 1) {              // TxPattern( ) change
      col = vec3 (0.1,0.4,0.1) * (1. + Tx2Pattern ((ro.y < 3.5) ? ro : 2. * ro));
      if (ro.y < 3.2) col *= vec3 (0.9*(1.+cos(tCur*0.3)), 0.4, 0.9*(1.+sin(tCur*0.4)));
        ///////////////////////////////////////////// eGlow ad
      ...
      vn = VaryNf (1. * ro, vn, 0.92);
    } else if (idObjT == 2) {
      col = vec3 (0.1*(1.+5.*sin(tCur*0.2)), 0.1, 0.1*(1.+5.*sin(tCur*0.4)))* (0.1 + Tx2Pattern ((ro.y < 3.5) ? ro : 2. * ro))  ; //( 1.+Tx2P( ) ) adition
     // De  vec3 (0.7, 0.7, 0.6)* (1.+    )
    ...
    col = mix (col, bgCol, 1. - exp (- dstObj));
                                                                            //  (5) 155~156line deleat
      ////////////////////////////////////////////////////////////////////////// Glow & Sparkling
    if (idObjT == 2 || idObj == 1 ) col += mix (vec3 (0., 0.3, 1.), vec3 (0., 1., 0.3), rnd) *
       (bGlow + 20. * eGlow * eGlow * eFlash) * (1. - smoothstep (0.4, 1., dstObj / dstFar));
      //////////////////////////////////////////////////////////////////////////
                                                                           // (6) Glow Sparkling
                                                                           // (7) color change
                   // col +=mix(vec3(1.0,0.3,9.),vec3(0.,0.,1.),rnd)......      //   exsamples color
