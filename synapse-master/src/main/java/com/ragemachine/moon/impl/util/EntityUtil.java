...
import com.ragemachine.moon.impl.client.modules.combat.Killaura;
...
public class EntityUtil implements Util
{
    ...
    public static BlockPos getPlayerPosMinus1Y(final EntityPlayer player) {
        return new BlockPos(Math.floor(player.posX), Math.floor(player.posY - 1), Math.floor(player.posZ));
    }
    ...
    public static boolean isntValid(final Entity entity, final double range) {
        return entity == null || isDead(entity) || entity.equals((Object)EntityUtil.mc.player) || (entity instanceof EntityPlayer && MoonHack.getFriendManager.isFriend(entity.getName())) || EntityUtil.mc.player.getDistanceSq(entity) > MathUtil.square(range);
    }
    ...
    public static Color getColor(final Entity entity, final int red, final int green, final int blue, final int alpha, final boolean colorFriends) {
        ...
        if (entity instanceof EntityPlayer) {
            if (colorFriends && MoonHack.getFriendManager.isFriend((EntityPlayer)entity)) {
                color = new Color(0.33333334f, 1.0f, 1.0f, alpha / 255.0f);
            }
            final Killaura killaura = MoonHack.getModuleManager.getModule(Killaura.class);
    ...
    public static Map<String, Integer> getTextRadarPlayers() {
        ...
        for (final EntityPlayer player : EntityUtil.mc.world.playerEntities) {
            ...
            output.put(healthSB.toString() + " " + (MoonHack.getFriendManager.isFriend(player) ? "§b" : "§r") + player.getName() + " " + distanceSB.toString() + " " + "§f" + MoonHack.getTotemManager.getTotemPopString(player) + MoonHack.POTION_MANAGER.getTextRadarPotion(player), (int)EntityUtil.mc.player.getDistance((Entity)player));
    ...
    public static void faceVectorPacketInstant(Vec3d var0) {
        ...
        Minecraft.getMinecraft().player.connection.sendPacket(new CPacketPlayer.Rotation(var1[0], var1[1], Minecraft.getMinecraft().player.onGround));
    }
    ...
    private static float[] getLegitRotations(Vec3d var0) {
        ...
        return new float[]{Minecraft.getMinecraft().player.rotationYaw + MathHelper.wrapDegrees(var10 - Minecraft.getMinecraft().player.rotationYaw), Minecraft.getMinecraft().player.rotationPitch + MathHelper.wrapDegrees(var11 - Minecraft.getMinecraft().player.rotationPitch)};
    }
